                        ...
                        @foreach($contactInfo as $info)
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="font-semibold text-secondary-900 dark:text-white mb-1">{{ $info['title'] }}</h3>
                                    @foreach($info['details'] as $detail)
                                        <p class="text-sm text-secondary-500 dark:text-secondary-400">{{ $detail }}</p>
                                    @endforeach
                                    <p class="text-xs text-secondary-400 dark:text-secondary-500 mt-1">{{ $info['description'] }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        @foreach($features as $index => $feature)
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <h4 class="font-medium text-sm text-secondary-900 dark:text-white">{{ $feature['title'] }}</h4>
                                    <p class="text-xs text-secondary-500 dark:text-secondary-400">{{ $feature['description'] }}</p>
                                </div>
                            </div>
                            @if(!$loop->last)
        ...
        <div class="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            ...
            @foreach($faqs as $faq)
                <div class="card-hover">
                    <h3 class="font-semibold text-secondary-900 dark:text-white mb-2">{{ $faq['question'] }}</h3>
                    <p class="text-sm text-secondary-500 dark:text-secondary-400">{{ $faq['answer'] }}</p>
                </div>
            @endforeach
        </div>
    </div>
