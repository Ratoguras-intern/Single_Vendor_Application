...
@php
    ...
    ])->filter(fn ($v) => filled($v['title']) || filled($v['text']));
...
<section class="py-12 sm:py-16 lg:py-20 bg-secondary-50 dark:bg-secondary-950">
    <div class="section max-w-6xl mx-auto">
        <div class="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            @if ($hasStory)
                <article class="card p-6 sm:p-8 lg:p-10 order-2 lg:order-1">
                    <div class="page-content text-secondary-600 dark:text-secondary-400">{!! $content !!}</div>
                </article>
            @endif

            <div class="order-1 lg:order-2 space-y-4 {{ $hasStory ? '' : 'w-full max-w-xl mx-auto' }}">
                <div class="overflow-hidden rounded-2xl shadow-card group">
                    <img src="{{ asset('images/pages/about-collab.jpg') }}" alt="Our team collaborating in the studio" loading="lazy" class="w-full aspect-[16/10] object-cover group-hover:scale-[1.03] transition-transform duration-500">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="overflow-hidden rounded-2xl shadow-card group">
                        <img src="{{ asset('images/pages/shopping-bags.jpg') }}" alt="Orders packed and ready for our customers" loading="lazy" class="w-full h-full min-h-[160px] object-cover group-hover:scale-[1.03] transition-transform duration-500">
                    </div>
                    <div class="rounded-2xl bg-gradient-to-br from-primary-500 to-primary-600 text-white flex flex-col justify-center p-6 shadow-card">
                        <svg class="h-7 w-7 mb-3 opacity-90" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 0 0 2.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456Z"/></svg>
                        <p class="font-bold leading-snug">{{ $badgeTitle }}</p>
                        @if (filled($badgeSub))
                            <p class="text-xs text-primary-100 mt-1.5">{{ $badgeSub }}</p>
                        @endif
                    </div>
                </div>
            </div>
        ...
        @if ($values->isNotEmpty())
            <div class="grid sm:grid-cols-3 gap-4 mt-12 lg:mt-16">
                @foreach ($values as $value)
                    <div class="card-hover flex items-start gap-3.5 !p-5">
                        <span class="w-10 h-10 rounded-xl bg-{{ $value['color'] }}-50 dark:bg-{{ $value['color'] }}-950/30 flex items-center justify-center shrink-0">
                            <svg class="h-5 w-5 text-{{ $value['color'] }}-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">{!! $valueIcons[$value['icon']] ?? $valueIcons['check'] !!}</svg>
                        </span>
                        <span>
...
@if ($stats->isNotEmpty())
    <section class="py-12 sm:py-14 bg-white dark:bg-secondary-900 border-y border-secondary-200 dark:border-secondary-800">
        <div class="section">
            <dl class="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
                @foreach ($stats as $stat)
                    <div>
                        <dd class="text-3xl sm:text-4xl font-extrabold text-primary-500 dark:text-primary-400 tabular-nums">{{ number_format($stat['value']) }}+</dd>
                        <dt class="mt-1 text-sm font-medium text-secondary-500 dark:text-secondary-400">{{ $stat['label'] }}</dt>
                    </div>
                @endforeach
            </dl>
        </div>
