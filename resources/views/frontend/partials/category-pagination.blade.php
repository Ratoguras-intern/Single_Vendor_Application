...
@if ($paginator->hasPages())
    <nav class="flex items-center justify-center gap-2" role="navigation" aria-label="Pagination">
        ...
        @else
            <button type="button" wire:click="setPage({{ $paginator->currentPage() - 1 }})" class="inline-flex items-center justify-center w-10 h-10 rounded-btn border border-secondary-200 dark:border-secondary-700 text-secondary-600 dark:text-secondary-400 hover:bg-secondary-50 dark:hover:bg-white/5 hover:text-secondary-900 dark:hover:text-white transition-colors" rel="prev" aria-label="Previous page">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5"/></svg>
            </button>
        @endif
        ...
        @foreach($pages as $page)
            ...
            @else
                <button type="button" wire:click="setPage({{ $page }})" class="inline-flex items-center justify-center w-10 h-10 rounded-btn border border-secondary-200 dark:border-secondary-700 text-secondary-600 dark:text-secondary-400 hover:bg-secondary-50 dark:hover:bg-white/5 hover:text-secondary-900 dark:hover:text-white transition-colors text-sm" aria-label="Page {{ $page }}">{{ $page }}</button>
            @endif
        @endforeach
