    ...
    @foreach($bottomBanners as $banner)
        ...
        <section class="border-t border-secondary-200 dark:border-secondary-800 {{ $visibility }}" @if($banner->section_margin_css) style="{{ $banner->section_margin_css }}" @endif
            ...
            <div class="section py-0">
                <a href="{{ $banner->link ?? '#' }}" class="group relative block w-full overflow-hidden bg-secondary-900 min-h-[160px] sm:min-h-[200px] lg:min-h-[250px]" @if($tileStyle) style="{{ $tileStyle }}" @endif>
                    @if($banner->image_url)
                    ...
                    <div class="relative h-full flex {{ $verticalClass }} px-6 sm:px-10 lg:px-14 py-8 sm:py-10" @if($banner->content_padding_css) style="{{ $banner->content_padding_css }}" @endif>
                        <div class="max-w-xl flex flex-col {{ $alignClass }}" @if($textWidthStyle) style="{{ $textWidthStyle }}" @endif>
                            ...
                            @endif
                            @if($banner->title)
                                <h2 class="text-xl sm:text-2xl lg:text-3xl font-extrabold leading-tight {{ $textColor }}">{{ $banner->title }}</h2>
                            @endif
