    ...
    @foreach($bottomBanners as $banner)
        ...
        <section class="border-t border-secondary-200 dark:border-secondary-800"
            ...
            <div class="section py-0">
                <a href="{{ $banner->link ?? '#' }}" class="group relative block w-full overflow-hidden bg-secondary-900 min-h-[160px] sm:min-h-[200px] lg:min-h-[250px]">
                    @if($banner->image_url)
                    ...
                    @endif
                    <div class="absolute inset-0" @if($overlayStyle) style="background: linear-gradient(to right, {{ $overlayStyle }} 40%, transparent 100%);" @else class="absolute inset-0 bg-gradient-to-r from-black/60 via-black/20 to-transparent" @endif></div>
