        ...
        <div :class="viewMode === 'list' ? 'flex-1 p-4 flex flex-col justify-between min-w-0' : 'p-3 space-y-1.5 flex flex-col flex-1 mt-auto'">
            <div :class="viewMode === 'list' ? 'space-y-1' : ''">
                @if(!empty($product['brand']))
                    <p class="text-[10px] font-medium text-primary-600 dark:text-primary-400 uppercase tracking-wider">{{ $product['brand'] }}</p>
                @endif
            ...
            <div x-show="viewMode !== 'list'" class="flex flex-col gap-1.5 pt-0.5">
                ...
                <button x-on:click="$store.cart.buyNow({ id: {{ $product['id']}}, name: '{{ addslashes($product['name']) }}', price: {{ $product['price']}}, image: '{{ $product['image'] }}' })" class="btn-outline w-full btn-sm hidden group-hover:flex" @if($isOutOfStock) disabled @endif>
