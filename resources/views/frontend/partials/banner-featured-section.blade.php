@if(!empty($featuredBanners) && $featuredBanners->isNotEmpty())
    @foreach($featuredBanners as $banner)
        ...
        <section class="py-4 sm:py-6 lg:py-8 border-t border-secondary-200 dark:border-secondary-800 {{ $visibility }}" @if($banner->section_margin_css) style="{{ $banner->section_margin_css }}" @endif
            ...
            x-init="init()">
            <div class="section">
                <div class="group relative flex w-full overflow-hidden rounded-card bg-secondary-900 min-h-[200px] sm:min-h-[250px] lg:min-h-[300px]" @if($tileStyle) style="{{ $tileStyle }}" @endif>
