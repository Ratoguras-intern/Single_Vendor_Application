@if(!empty($promotionalBanners) && $promotionalBanners->isNotEmpty())
    @foreach($promotionalBanners as $banner)
        ...
        @endphp
        <section class="py-4 sm:py-6 lg:py-8 {{ $visibility }}" @if($banner->section_margin_css) style="{{ $banner->section_margin_css }}" @endif
            ...
            <div class="section">
                <a href="{{ $banner->link ?? '#' }}" class="group relative block w-full overflow-hidden rounded-card bg-secondary-900 min-h-[180px] sm:min-h-[220px] lg:min-h-[260px]" @if($tileStyle) style="{{ $tileStyle }}" @endif>
                    @if($banner->image_url)
                    ...
                    @if($banner->overlay_enabled)
                        <div class="absolute inset-0" style="background: linear-gradient(to right, {{ $banner->overlay_rgba }} 40%, transparent 100%);"></div>
                    @endif
                    <div class="relative h-full flex {{ $verticalClass }} px-6 sm:px-10 lg:px-14 py-8 sm:py-10" @if($banner->content_padding_css) style="{{ $banner->content_padding_css }}" @endif>
                        <div class="max-w-xl flex flex-col {{ $alignClass }}" @if($textWidthStyle) style="{{ $textWidthStyle }}" @endif>
                            ...
                            @if($banner->button_text || $banner->secondary_button_text)
                                <div class="mt-4 flex flex-wrap gap-3 {{ $banner->text_alignment === 'center' ? 'justify-center' : '' }} {{ $banner->text_alignment === 'right' ? 'justify-end' : '' }}">
