@if(!empty($promotionalBanners) && $promotionalBanners->isNotEmpty())
    @foreach($promotionalBanners as $banner)
        ...
        <section class="py-8 sm:py-10 lg:py-12"
            ...
            <div class="section">
                <a href="{{ $banner->link ?? '#' }}" class="group relative block w-full overflow-hidden rounded-card bg-secondary-900 min-h-[180px] sm:min-h-[220px] lg:min-h-[260px]">
                    @if($banner->image_url)
                    ...
                    @endif
                    <div class="absolute inset-0" @if($overlayStyle) style="background: linear-gradient(to right, {{ $overlayStyle }} 40%, transparent 100%);" @else class="absolute inset-0 bg-gradient-to-r from-black/70 via-black/30 to-transparent" @endif></div>
