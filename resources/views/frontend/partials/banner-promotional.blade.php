@if(!empty($promotionalBanners) && $promotionalBanners->isNotEmpty())
    @foreach($promotionalBanners as $banner)
        ...
        <section class="py-4 sm:py-6 lg:py-8 {{ $visibility }}" @if($banner->section_margin_css) style="{{ $banner->section_margin_css }}" @endif
            ...
            x-init="init()">
            <div class="section">
                <div class="group relative flex w-full overflow-hidden rounded-card bg-secondary-900 min-h-[180px] sm:min-h-[220px] lg:min-h-[260px]" @if($tileStyle) style="{{ $tileStyle }}" @endif>
