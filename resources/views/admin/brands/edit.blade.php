    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                ...
                <div>
                    <x-media-picker.picker name="logo_media_id" label="Logo" folder="logos" remove-name="remove_logo"
                        :value="media_for_path($brand->logo)?->id"
                        :preview="$brand->logo ? media_url($brand->logo) : null"
                        help="Choose from the media library or upload a new logo from your computer." />
                    @error('logo_media_id')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
