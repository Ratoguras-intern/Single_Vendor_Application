                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.brands.destroy', $brand) }}" method="POST" x-data @submit.prevent="$store.confirmModal.open({ title: 'Delete Brand', message: 'Are you sure you want to delete this brand?', form: $el })">
                                        @csrf
                                        @method('DELETE')
...
function toggleBrand(id) {
    fetch(`/admin/brands/${id}/toggle-status`, {
        ...
        window.dispatchEvent(new CustomEvent('toast', { detail: { type: 'error', message: 'Failed to update brand status.' } }));
    });
}
...
function bulkDeleteBrands() {
    ...
    if (ids.length === 0) {
        window.dispatchEvent(new CustomEvent('toast', { detail: { type: 'warning', message: 'Please select at least one brand.' } }));
        return;
    }

    Alpine.store('confirmModal').open({
        title: 'Delete Brands',
        message: `Delete ${ids.length} selected brand(s)? This action cannot be undone.`,
        onConfirm: async () => {
            try {
                const r = await fetch('{{ route('admin.brands.bulkDestroy') }}', {
                    method: 'DELETE',
                    headers: {
