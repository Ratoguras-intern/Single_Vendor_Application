...
@php
    $breadcrumbs = [
        ...
        ['label' => $page->title, 'url' => route('admin.pages.show', $page)],
        ['label' => 'Edit', 'url' => null],
    ];
@endphp
...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-800 dark:text-white">Edit Page</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update page content, visibility, and search engine information.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.pages.show', $page) }}" class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $page->subtitle) }}"
                        ...
                        <div>
                            ...
                            <div class="mt-1.5 flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500">
                                ...
                                <span>URL preview: <span id="slug-preview" class="font-medium text-gray-600 dark:text-gray-300">/{{ old('slug', $page->slug) }}</span></span>
                            </div>
                            @error('slug')
                                <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="short_description" id="short_description" rows="2"
                                ...
                                placeholder="A short summary displayed in page headers or search metadata.">{{ old('short_description', $page->short_description) }}</textarea>
                            <div class="mt-1.5 flex items-center justify-between text-xs text-gray-400 dark:text-gray-500">
                                ...
                                <span id="desc-count" class="tabular-nums">{{ strlen(old('short_description', $page->short_description ?? '')) }}/500</span>
                            </div>
                ...
                <div class="flex flex-col rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden dark:border-gray-800 dark:bg-white/[0.03]" style="height: 750px;">
                    ...
                    <textarea name="content" id="content-hidden" class="hidden">{!! old('content', $page->content) !!}</textarea>
            ...
            <div class="space-y-6">
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select name="template" id="template"
                                ...
                                @foreach (\App\Models\Page::TEMPLATES as $value => $label)
                                    <option value="{{ $value }}" {{ old('template', $page->template) === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" id="footer-toggle" role="switch" aria-checked="{{ old('show_in_footer', $page->show_in_footer ? '1' : '0') === '1' ? 'true' : 'false' }}"
                                class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center overflow-hidden rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 {{ old('show_in_footer', $page->show_in_footer ? '1' : '0') === '1' ? 'bg-brand-500' : 'bg-gray-300 dark:bg-gray-600' }}">
                                <span class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ old('show_in_footer', $page->show_in_footer ? '1' : '0') === '1' ? 'translate-x-5' : 'translate-x-0' }}"></span>
                            </button>
                            <input type="hidden" name="show_in_footer" id="show_in_footer" value="{{ old('show_in_footer', $page->show_in_footer ? '1' : '0') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="footer_section" id="footer_section"
                                ...
                                <option value="customer-care" {{ old('footer_section', $page->footer_section) === 'customer-care' ? 'selected' : '' }}>Customer Care</option>
                                <option value="company" {{ old('footer_section', $page->footer_section) === 'company' ? 'selected' : '' }}>Company</option>
                                <option value="legal" {{ old('footer_section', $page->footer_section) === 'legal' ? 'selected' : '' }}>Legal</option>
                            </select>
                        </div>

                        <div>
                            <label for="footer_order" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Display Order</label>
                            <input type="number" name="footer_order" id="footer_order" value="{{ old('footer_order', $page->footer_order) }}" min="0"
                                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="mt-1.5 flex items-center justify-between text-xs text-gray-400 dark:text-gray-500">
                                    ...
                                    <span id="seo-title-count" class="tabular-nums">{{ strlen(old('seo_title', $page->seo_title ?? '')) }}/60</span>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="mt-1.5 flex items-center justify-between text-xs text-gray-400 dark:text-gray-500">
                                    ...
                                    <span id="seo-desc-count" class="tabular-nums">{{ strlen(old('seo_description', $page->seo_description ?? '')) }}/160</span>
                                </div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <textarea name="og_description" id="og_description" rows="2"
                                    class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white"
                                    placeholder="Overrides SEO description on social platforms">{{ old('og_description', $page->og_description) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div>
                        @php
                            $ogMedia = $page->og_image
                                ? \App\Models\Media::where('path', $page->og_image)->first()
                                : null;
                        @endphp
                        <x-media-picker.picker name="og_image_media_id" folder="pages"
                            :value="$ogMedia?->id" :preview="$page->og_image_url"
                            remove-name="remove_og_image"
                            help="Falls back to featured image or site default." />
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div>
                        @php
                            $featuredMedia = $page->featured_image
                                ? \App\Models\Media::where('path', $page->featured_image)->first()
                                : null;
                        @endphp
                        <x-media-picker.picker name="featured_image_media_id" folder="pages"
                            :value="$featuredMedia?->id" :preview="$page->featured_image_url"
    ...
    <script type="text/turbo-script">
    {
        ...
        const footerCheckbox = document.querySelector('input[name="show_in_footer"][type="hidden"]');
