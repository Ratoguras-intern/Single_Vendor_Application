    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                ...
                <div>
                    @php $maxNewImages = max(0, 5 - $product->images->count()); @endphp
                    @if ($maxNewImages === 0)
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Add More Images</label>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Image limit reached. Remove an image above to add a new one.</p>
                    @else
                        <x-media-picker.picker name="gallery_media_ids" label="Add More Images ({{ $maxNewImages }} of 5 slots remaining)"
                            :multiple="true" :max="$maxNewImages" folder="products"
                            help="Select from the media library or upload new images." />
                        @error('gallery_media_ids')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    @endif
                </div>
