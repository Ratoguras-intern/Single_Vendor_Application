...
@section('content')
    <div class="mb-6">
        <h2 class="text-xl font-bold text-gray-800 dark:text-white">Edit FAQ Category</h2>
    </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $faqCategory->slug) }}" placeholder="Leave empty to auto-generate"
                        class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                    @error('slug')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $faqCategory->sort_order ?? 0) }}" min="0"
                        class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                    @error('sort_order')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center gap-2 pt-1">
                    ...
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $faqCategory->is_active) ? 'checked' : '' }}
