    ...
    <div class="mb-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
        <form action="{{ route('admin.banners.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="min-w-0 w-full sm:w-auto sm:min-w-[150px]">
                ...
                <select name="position" id="position" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                    ...
                    @foreach($positions as $pos)
                        <option value="{{ $pos }}" {{ $currentPosition === $pos ? 'selected' : '' }}>{{ ucwords(str_replace('-', ' ', $pos)) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.banners.index') }}" class="rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="w-full" id="banner-table">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800 sortable-container" id="sortable-container">
                    @forelse($banners as $banner)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] sortable-row" data-id="{{ $banner->id }}">
                            <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400">{{ $banner->id }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                @if($banner->image)
                                    <img src="{{ $banner->image_url }}" alt="{{ $banner->title }}" class="h-12 w-24 rounded-lg object-cover">
                                @else
                            ...
                            <td class="px-5 py-4 text-sm font-medium text-gray-800 dark:text-white">{{ $banner->title ?? '—' }}</td>
                            <td class="px-5 py-4">
                                ...
                                @if($banner->target_pages)
                                    <div class="mt-1 flex flex-wrap gap-1">
                                        @foreach(array_diff($banner->target_pages ?? [], [$banner->position]) as $page)
                                            <span class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400">{{ ucfirst($page) }}</span>
                                        @endforeach
                                    </div>
                                @endif
                            </td>
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    <button onclick="toggleBanner({{ $banner->id }})" class="relative inline-flex h-6 w-11 shrink-0 items-center overflow-hidden rounded-full transition-colors
                                        {{ $banner->is_enabled ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600' }}">
                                        <span class="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform
                                            {{ $banner->is_enabled ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                    </button>
                                    <span class="text-xs font-medium
                                        {{ $banner->status === 'active' ? 'text-green-600 dark:text-green-400' : '' }}
                                        {{ $banner->status === 'expired' ? 'text-red-600 dark:text-red-400' : '' }}
                                        {{ $banner->status === 'scheduled' ? 'text-amber-600 dark:text-amber-400' : '' }}
                                        {{ $banner->status === 'inactive' ? 'text-gray-500 dark:text-gray-400' : '' }}">
                            ...
                            <td class="px-5 py-4 text-xs text-gray-500 dark:text-gray-400">
                                @if($banner->starts_at || $banner->ends_at)
                                    <div class="flex flex-col gap-0.5">
                                        <span>Start: {{ $banner->starts_at ? $banner->starts_at->format('M d, Y H:i') : 'Immediate' }}</span>
                                        <span>End: {{ $banner->ends_at ? $banner->ends_at->format('M d, Y H:i') : 'Never' }}</span>
                                    </div>
                                @else
                                    Always
                                @endif
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="{{ route('admin.banners.show', $banner) }}" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
                        ...
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <a href="{{ route('admin.banners.create') }}" class="inline-flex items-center gap-1.5 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">
