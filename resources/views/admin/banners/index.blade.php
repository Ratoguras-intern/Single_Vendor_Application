        ...
        <form action="{{ route('admin.banners.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
            <div class="min-w-[150px]">
                ...
                <select name="position" id="position" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                    ...
                    <option value="promotional" {{ request('position') === 'promotional' ? 'selected' : '' }}>Promotional</option>
                    <option value="middle" {{ request('position') === 'middle' ? 'selected' : '' }}>Middle</option>
                    <option value="collection" {{ request('position') === 'collection' ? 'selected' : '' }}>Collection</option>
                </select>
            </div>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            ...
                            <td class="px-5 py-4">
                                <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium
                                    {{ ($banner->position === 'hero' ? 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400' :
                                       ($banner->position === 'promotional' ? 'bg-purple-50 text-purple-700 dark:bg-purple-500/10 dark:text-purple-400' :
                                       ($banner->position === 'middle' ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' :
                            ...
                            <td class="px-5 py-4 text-xs text-gray-500 dark:text-gray-400">
                                @if($banner->starts_at || $banner->ends_at)
                                    {{ $banner->starts_at ? $banner->starts_at->format('M d') : 'Now' }} — {{ $banner->ends_at ? $banner->ends_at->format('M d') : '∞' }}
                                @else
                                    Always
                                @endif
                            </td>
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="{{ route('admin.banners.edit', $banner) }}" class="text-brand-500 hover:text-brand-600">
