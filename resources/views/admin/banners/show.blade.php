...
@section('content')
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
            <form action="{{ route('admin.banners.duplicate', $banner) }}" method="POST" class="inline">
                @csrf
            ...
            <a href="{{ route('admin.banners.edit', $banner) }}" class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600">
            ...
            <form action="{{ route('admin.banners.destroy', $banner) }}" method="POST" x-data @submit.prevent="$store.confirmModal.open({ title: 'Delete Banner', message: 'Delete this banner permanently?', form: $el })">
                @csrf @method('DELETE')
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="p-6">
                    @if($banner->image)
                        <div class="mb-6">
                            <img src="{{ $banner->image_url }}" alt="{{ $banner->title }}" class="rounded-lg max-h-48 w-full object-cover">
                        </div>
                    @endif
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div>
                            <dt class="text-gray-500 dark:text-gray-400">Title</dt>
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">{{ $banner->title ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500 dark:text-gray-400">Subtitle</dt>
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">{{ $banner->subtitle ?? '—' }}</dd>
                        </div>
                        <div class="sm:col-span-2">
                            <dt class="text-gray-500 dark:text-gray-400">Description</dt>
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">{{ $banner->description ?? '—' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            @if($banner->image)
                                <img src="{{ $banner->image_url }}" alt="" class="rounded-lg max-h-40 w-full object-cover">
                            @else
                        ...
                        <div>
                            ...
                            @if($banner->mobile_image)
                                <img src="{{ $banner->mobile_image_url }}" alt="" class="rounded-lg max-h-40 w-full object-cover">
                            @else
                ...
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">{{ $banner->button_text ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500 dark:text-gray-400">Primary Button Link</dt>
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">{{ $banner->link ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500 dark:text-gray-400">Secondary Button Text</dt>
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">{{ $banner->secondary_button_text ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500 dark:text-gray-400">Secondary Button URL</dt>
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">{{ $banner->secondary_button_url ?? '—' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
                        ...
                        <div>
                            ...
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">
                                <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400">
                                    {{ ucwords(str_replace('-', ' ', $banner->position)) }}
                                </span>
                            </dd>
                        </div>
                        <div>
                            ...
                            <dd class="font-medium text-gray-800 dark:text-white mt-0.5">
                                @if($banner->target_pages)
                                    <div class="flex flex-wrap gap-1.5">
                                        @foreach($banner->target_pages as $page)
                                            <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300">{{ ucfirst($page) }}</span>
                                        @endforeach
                                    </div>
                                @else
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold
                            {{ $banner->is_enabled ? 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400' : 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400' }}">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold
                            {{ $banner->status === 'active' ? 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400' : '' }}
                            {{ $banner->status === 'expired' ? 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400' : '' }}
                            {{ $banner->status === 'scheduled' ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' : '' }}
                            {{ $banner->status === 'inactive' ? 'bg-gray-50 text-gray-700 dark:bg-gray-500/10 dark:text-gray-400' : '' }}">
                            {{ ucfirst($banner->status) }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-500 dark:text-gray-400">Sort Order</span>
                        <span class="text-sm font-medium text-gray-800 dark:text-white">{{ $banner->sort_order }}</span>
                    </div>
                </div>
            </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="space-y-6">
                            <div class="relative pl-8">
                                ...
                                <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $banner->starts_at?->format('M d, Y H:i') ?? 'Immediate' }}</p>
                            </div>
                            <div class="relative pl-8">
                                <div class="absolute left-0 top-1.5 h-5 w-5 rounded-full border-2
                                    {{ $banner->status === 'expired' ? 'border-red-500 bg-red-50 dark:bg-red-500/10' : 'border-gray-400 bg-gray-50 dark:bg-gray-800' }}
                                    flex items-center justify-center">
                                    <div class="h-2 w-2 rounded-full {{ $banner->status === 'expired' ? 'bg-red-500' : 'bg-gray-400' }}"></div>
                                </div>
                                <p class="text-xs text-gray-500 dark:text-gray-400">End</p>
                                <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $banner->ends_at?->format('M d, Y H:i') ?? 'Never' }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-6">
                    <dl class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <dd>
                                @if($banner->badge)
                                    <span class="inline-flex items-center rounded-full {{ $banner->badge_color ?? 'bg-green-500' }} px-2 py-0.5 text-xs font-bold text-white">{{ $banner->badge }}</span>
                                @else
                                    <span class="text-gray-400">—</span>
                                @endif
                            </dd>
                        ...
                        <div class="flex justify-between">
                            ...
                            <dd class="font-medium text-gray-800 dark:text-white capitalize">{{ $banner->text_alignment ?? 'Left' }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500 dark:text-gray-400">Overlay Opacity</dt>
                            <dd class="font-medium text-gray-800 dark:text-white">{{ $banner->overlay_opacity ?? '40' }}%</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500 dark:text-gray-400">Text Color</dt>
                            <dd class="font-medium text-gray-800 dark:text-white">{{ $banner->text_color ?? 'Default (White)' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
                ...
                <div class="p-6">
                    <dl class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <dd class="font-medium text-gray-800 dark:text-white">#{{ $banner->id }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500 dark:text-gray-400">Created</dt>
                            <dd class="font-medium text-gray-800 dark:text-white">{{ $banner->created_at->format('M d, Y H:i') }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500 dark:text-gray-400">Updated</dt>
                            <dd class="font-medium text-gray-800 dark:text-white">{{ $banner->updated_at->format('M d, Y H:i') }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
