                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] sortable-row" data-id="{{ $saleBanner->id }}">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    <button onclick="toggleSaleBanner({{ $saleBanner->id }})" class="relative inline-flex h-6 w-11 shrink-0 items-center overflow-hidden rounded-full transition-colors
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.sale-banners.destroy', $saleBanner) }}" method="POST" x-data @submit.prevent="$store.confirmModal.open({ title: 'Delete Sale Banner', message: 'Are you sure you want to delete this sale banner?', form: $el })">
                                        @csrf
                                        @method('DELETE')
