...
<?php $__env->startSection('content'); ?>
    ...
    <div class="mb-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
        <form action="<?php echo e(route('admin.products.index')); ?>" method="GET" class="flex flex-wrap items-end gap-4">
            <div class="flex-1 min-w-[200px]">
                <label for="search" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Search</label>
                <input type="text" name="search" id="search" value="<?php echo e(request('search')); ?>" placeholder="Search by name, slug, SKU..."
            ...
            <div class="min-w-[150px]">
                ...
                <select name="visibility" id="visibility"
                    class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                    <option value="">All</option>
                    <option value="is_featured" <?php echo e(request('visibility') === 'is_featured' ? 'selected' : ''); ?>>Featured</option>
                    <option value="is_new_arrival" <?php echo e(request('visibility') === 'is_new_arrival' ? 'selected' : ''); ?>>New Arrivals</option>
                    <option value="is_trending" <?php echo e(request('visibility') === 'is_trending' ? 'selected' : ''); ?>>Trending</option>
                    <option value="is_best_seller" <?php echo e(request('visibility') === 'is_best_seller' ? 'selected' : ''); ?>>Best Sellers</option>
                    <option value="is_flash_sale" <?php echo e(request('visibility') === 'is_flash_sale' ? 'selected' : ''); ?>>Flash Sales</option>
                    <option value="is_recommended" <?php echo e(request('visibility') === 'is_recommended' ? 'selected' : ''); ?>>Recommended</option>
                    <option value="is_popular" <?php echo e(request('visibility') === 'is_popular' ? 'selected' : ''); ?>>Popular</option>
                    <option value="is_limited_edition" <?php echo e(request('visibility') === 'is_limited_edition' ? 'selected' : ''); ?>>Limited Edition</option>
                </select>
            </div>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                    <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400"><?php echo e($product->id); ?></td>
                            <td class="px-5 py-4">
                                <?php if($product->primaryImage()): ?>
                                    <img src="<?php echo e(asset('storage/' . $product->primaryImage()->image)); ?>" alt="<?php echo e($product->name); ?>" class="h-10 w-10 rounded-lg object-cover">
                                <?php else: ?>
                            ...
                            <td class="px-5 py-4 text-sm font-medium text-gray-800 dark:text-white">
                                <a href="<?php echo e(route('admin.products.show', $product)); ?>" class="hover:text-brand-500"><?php echo e($product->name); ?></a>
                            </td>
                            <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400"><?php echo e($product->sku); ?></td>
                            <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400"><?php echo e($product->category->name ?? '-'); ?></td>
                            <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400">
                                $<?php echo e(number_format($product->price, 2)); ?>

                                <?php if($product->discount_price): ?>
                                    <span class="ml-1 text-xs text-red-500">$<?php echo e(number_format($product->discount_price, 2)); ?></span>
                                <?php endif; ?>
                            </td>
                            <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400"><?php echo e($product->stock); ?></td>
                            <td class="px-5 py-4">
                                <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium
                                    <?php echo e($product->status === 'active' ? 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400' : 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400'); ?>">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex flex-wrap gap-1">
                                    <?php if($product->is_featured): ?>
                                        <button onclick="toggleProductFlag(<?php echo e($product->id); ?>, 'is_featured')" class="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-700 hover:bg-blue-100 dark:bg-blue-500/10 dark:text-blue-400" title="Click to toggle">Featured</button>
                                    <?php endif; ?>
                                    <?php if($product->is_new_arrival): ?>
                                        <button onclick="toggleProductFlag(<?php echo e($product->id); ?>, 'is_new_arrival')" class="inline-flex items-center gap-1 rounded-full bg-green-50 px-2 py-0.5 text-[10px] font-semibold text-green-700 hover:bg-green-100 dark:bg-green-500/10 dark:text-green-400" title="Click to toggle">New</button>
                                    <?php endif; ?>
                                    <?php if($product->is_trending): ?>
                                        <button onclick="toggleProductFlag(<?php echo e($product->id); ?>, 'is_trending')" class="inline-flex items-center gap-1 rounded-full bg-purple-50 px-2 py-0.5 text-[10px] font-semibold text-purple-700 hover:bg-purple-100 dark:bg-purple-500/10 dark:text-purple-400" title="Click to toggle">Trending</button>
                                    <?php endif; ?>
                                    <?php if($product->is_best_seller): ?>
                                        <button onclick="toggleProductFlag(<?php echo e($product->id); ?>, 'is_best_seller')" class="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-700 hover:bg-amber-100 dark:bg-amber-500/10 dark:text-amber-400" title="Click to toggle">Best Seller</button>
                                    <?php endif; ?>
                                    <?php if($product->is_flash_sale): ?>
                                        <button onclick="toggleProductFlag(<?php echo e($product->id); ?>, 'is_flash_sale')" class="inline-flex items-center gap-1 rounded-full bg-red-50 px-2 py-0.5 text-[10px] font-semibold text-red-700 hover:bg-red-100 dark:bg-red-500/10 dark:text-red-400" title="Click to toggle">Flash</button>
                                    <?php endif; ?>
                                    <?php if($product->is_recommended): ?>
                                        <button onclick="toggleProductFlag(<?php echo e($product->id); ?>, 'is_recommended')" class="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-semibold text-indigo-700 hover:bg-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-400" title="Click to toggle">Recommended</button>
                                    <?php endif; ?>
                                    <?php if($product->is_popular): ?>
                                        <button onclick="toggleProductFlag(<?php echo e($product->id); ?>, 'is_popular')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold text-gray-700 hover:bg-gray-200 dark:bg-gray-500/10 dark:text-gray-400" title="Click to toggle">Popular</button>
                                    <?php endif; ?>
                                    <?php if($product->is_limited_edition): ?>
                                        <button onclick="toggleProductFlag(<?php echo e($product->id); ?>, 'is_limited_edition')" class="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-800 hover:bg-amber-200 dark:bg-amber-500/10 dark:text-amber-300" title="Click to toggle">Limited</button>
                                    <?php endif; ?>
                                    <?php if(!$product->is_featured && !$product->is_new_arrival && !$product->is_trending && !$product->is_best_seller && !$product->is_flash_sale && !$product->is_recommended && !$product->is_popular && !$product->is_limited_edition): ?>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="<?php echo e(route('admin.products.show', $product)); ?>" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
                                    ...
                                    <a href="<?php echo e(route('admin.products.edit', $product)); ?>" class="text-brand-500 hover:text-brand-600">
                        ...
                        <tr>
                            <td colspan="11" class="px-5 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <a href="<?php echo e(route('admin.products.create')); ?>" class="inline-flex items-center gap-1.5 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Co_laravel\Single Vendor Ecomm\single-vendor-ecommerce\resources\views\admin\products\index.blade.php ENDPATH**/ ?>