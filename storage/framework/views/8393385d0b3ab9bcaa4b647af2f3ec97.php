<div class="rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6 mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input type="text" name="config[company_name]" value="<?php echo e(old('config.company_name', $section->config['company_name'] ?? 'NBK Vertex')); ?>"
        ...
        <div>
            ...
            <input type="text" name="config[phone]" value="<?php echo e(old('config.phone', $section->config['phone'] ?? '+1 (555) 123-4567')); ?>"
                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
        </div>
        <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
            <input type="email" name="config[email]" value="<?php echo e(old('config.email', $section->config['email'] ?? 'hello@nbkvertex.com')); ?>"
                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
        </div>
        <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Address</label>
            <input type="text" name="config[address]" value="<?php echo e(old('config.address', $section->config['address'] ?? '123 Fashion Street, Style City, SC 12345')); ?>"
                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
        </div>
        <div class="md:col-span-2">
            ...
            <textarea name="config[company_description]" rows="2"
                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white"><?php echo e(old('config.company_description', $section->config['company_description'] ?? 'A powerful e-commerce management platform.')); ?></textarea>
        </div>
        <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Copyright Text</label>
            <input type="text" name="config[copyright_text]" value="<?php echo e(old('config.copyright_text', $section->config['copyright_text'] ?? 'All Rights Reserved.')); ?>"
                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
        </div>
    </div>
</div>
...
<div class="rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6 mb-6">
    ...
    <?php if(empty($section->config['footer_columns'])): ?>
...
<?php /**PATH D:\Co_laravel\Single Vendor Ecomm\single-vendor-ecommerce\resources\views\admin\homepage-sections\_premium-footer.blade.php ENDPATH**/ ?>