...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6" x-data="{ sort: '<?php echo e($sort); ?>' }">
    <div class="flex items-center gap-3">
        ...
        <p class="text-sm text-secondary-500 dark:text-secondary-400">
            <span x-show="total > 0">
                ...
                of <span x-text="total"><?php echo e(number_format($total)); ?></span> Products
            </span>
        </p>
    </div>
    ...
    <div class="flex items-center gap-3">
        <div class="relative" x-data="{ open: false }" @click.away="open = false">
            ...
            >
                <?php $__currentLoopData = [
                    ...
                    <a
                        href="<?php echo e(request()->fullUrlWithQuery(['sort' => $value, 'page' => null])); ?>"
                        <?php if($ajax): ?> @click.prevent="$dispatch('shop:apply', { url: '<?php echo e(request()->fullUrlWithQuery(['sort' => $value, 'page' => null])); ?>' })" <?php endif; ?>
                        class="flex items-center gap-2 px-4 py-2 text-sm hover:bg-secondary-50 dark:hover:bg-white/5 transition-colors <?php echo e(request('sort', 'latest') === $value ? 'text-primary-600 dark:text-primary-400 font-medium' : 'text-secondary-700 dark:text-secondary-300'); ?>"
                        role="option"
                        aria-selected="<?php echo e(request('sort', 'latest') === $value); ?>"
                    >
                        <?php if(request('sort', 'latest') === $value): ?>
        ...
        <div class="hidden sm:flex items-center border border-secondary-200 dark:border-secondary-700 rounded-btn overflow-hidden">
            <button
                type="button"
                x-on:click="viewMode = 'grid'; localStorage.setItem('<?php echo e($viewStorageKey); ?>', 'grid')"
            ...
            <button
                type="button"
                x-on:click="viewMode = 'list'; localStorage.setItem('<?php echo e($viewStorageKey); ?>', 'list')"
...
<?php /**PATH D:\Co_laravel\Single Vendor Ecomm\single-vendor-ecommerce\resources\views\frontend\partials\category-toolbar.blade.php ENDPATH**/ ?>